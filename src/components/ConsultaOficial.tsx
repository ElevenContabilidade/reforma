...
export function ConsultaOficial({ tipo, titulo, subtitulo, placeholder }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
      <h2 className="mb-1 text-base font-semibold text-stone-900">{titulo}</h2>
      <p className="mb-5 text-sm text-stone-500">{subtitulo}</p>
      ...
      <div className="relative mb-2">
        ...
        <input
          ...
          className="w-full rounded-lg border border-stone-300 bg-white py-2.5 pl-10 pr-3 text-sm text-stone-900 shadow-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20"
        />
      </div>
      ...
      {!termo.trim() && (
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-stone-300 bg-stone-50 px-6 py-14 text-center">
          ...
          <p className="text-sm text-stone-400">Digite um {tipo}, atividade ou palavra-chave para consultar o tratamento tributário.</p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                ...
                <li key={`${item.codigo}::${item.descricao}`}>
                  <button
                    ...
                    className={`w-full rounded-lg border px-3 py-2.5 text-left text-sm transition ${
                      selecionado?.codigo === item.codigo && selecionado?.descricao === item.descricao
                        ? 'border-brand-400 bg-brand-50'
                        : 'border-stone-200 hover:border-brand-200 hover:bg-brand-50/40'
                    }`}
                  >
                    <span className="font-mono text-xs font-semibold text-stone-400">{item.codigo}</span>
                    <p className="text-stone-800">{item.descricao}</p>
                  </button>
                </li>
              ))}
            </ul>
          ...
          <div>
            {ficha ? (
              ...
              <div className="flex h-full items-center justify-center rounded-xl border border-dashed border-stone-300 bg-stone-50 px-6 py-14 text-center text-sm text-stone-400">
