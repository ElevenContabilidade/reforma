  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-teal-200 bg-gradient-to-br from-teal-50 to-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-slate-500">Melhor opção para {dados.nomeCliente || 'este cliente'} em {dados.anoSimulacao}</p>
              <p className="text-xl font-semibold text-slate-900">{melhor.nomeExibicao}</p>
          ...
          <div className="flex items-center gap-2 rounded-xl bg-white px-4 py-3 shadow-sm">
            ...
            <div>
              <p className="text-xs text-slate-500">Economia frente à pior opção ({pior.nomeExibicao})</p>
        ...
        {resultados.map((r, idx) => (
          <button
            ...
            className={`rounded-2xl border p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${
              detalhado.regime === r.regime ? 'border-teal-400 ring-2 ring-teal-500/20' : 'border-slate-200 bg-white'
            }`}
          >
            ...
            <p className="text-sm text-slate-500">{r.nomeExibicao}</p>
            <p className="mt-1 text-lg font-semibold text-slate-900">{formatarMoeda(r.totalMensal)}</p>
            <p className="text-xs text-slate-400">{formatarMoeda(r.totalAnual)}/ano · {formatarPercentual(r.aliquotaEfetivaTotal)}</p>
          </button>
        ))}
      </div>

      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h3 className="mb-4 text-base font-semibold text-slate-900">Carga tributária mensal por regime</h3>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {resultados.map((r) => (
            <button
              ...
              className={`rounded-full px-3 py-1.5 text-sm font-medium transition ${
                detalhado.regime === r.regime ? 'text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
        ...
        <div className="mb-4 flex items-start gap-2 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {detalhado.linhas.map((l) => (
              <tr key={l.tributo} className="border-b border-slate-100 last:border-0">
                <td className="py-2.5 font-medium text-slate-800">{l.tributo}</td>
                <td className="py-2.5 text-slate-500">{l.descricao ?? '—'}</td>
                <td className="py-2.5 text-right text-slate-700">{formatarMoeda(l.valorMensal)}</td>
                <td className="py-2.5 text-right text-slate-700">{formatarMoeda(l.valorAnual)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t-2 border-slate-200 font-semibold text-slate-900">
              ...
              <td className="py-3 text-slate-500 font-normal">{formatarPercentual(detalhado.aliquotaEfetivaTotal)} da receita</td>
