  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      ...
      <div className="mt-4 border-t border-slate-100 pt-4">
        <dt className="mb-1.5 text-xs font-medium uppercase tracking-wide text-slate-400">Documentos fiscais aceitos</dt>
        <div className="flex flex-wrap gap-1.5">
          {classificacao.documentosAceitos.length > 0 ? (
            classificacao.documentosAceitos.map((doc) => (
              <span key={doc} className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600">
      ...
      <p className="mt-4 border-t border-slate-100 pt-4 text-sm text-slate-600">{classificacao.descricao}</p>
      ...
      {indops.length > 0 && (
        <details className="mt-4 border-t border-slate-100 pt-4">
          ...
          <div className="mt-2 flex items-start gap-2 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500">
          ...
          <ul className="mt-2 space-y-2">
            {indops.map((i) => (
              <li key={i.codigo} className="rounded-lg border border-slate-200 p-3 text-xs">
                <span className="font-mono font-semibold text-slate-400">{i.codigo}</span>
                <p className="font-medium text-slate-800">{i.caracteristica}</p>
                <p className="mt-1 text-slate-500">Local do fornecimento: {i.local}</p>
              </li>
            ))}
          </ul>
        </details>
