  ...
  return (
    <section className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
      <div className="mb-1 flex items-center justify-between">
        <h2 className="text-base font-semibold text-stone-900">Produtos e serviços da empresa</h2>
        <span className={`text-sm font-medium ${valido ? 'text-stone-500' : 'text-rose-600'}`}>
      ...
      <div className="space-y-4">
        {CATEGORIAS_ITENS.map((categoria) => {
          ...
          return (
            <div
              key={categoria.id}
              className={`rounded-xl border p-4 transition ${ativo ? 'border-brand-300 bg-brand-50/40' : 'border-stone-200'}`}
            >
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex gap-3">
                  <input
                    type="checkbox"
                    className="mt-1 h-4 w-4 shrink-0 rounded border-stone-300 text-brand-600 focus:ring-brand-500"
                    checked={ativo}
                    onChange={(e) => setPercentual(categoria.id, e.target.checked ? percentualDe(categoria.id) || 10 : 0)}
                  />
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-stone-900">{categoria.nome}</span>
                      <span
                    ...
                    <p className="mt-1 text-sm text-stone-500">{categoria.descricao}</p>
                    <p className="mt-1 text-xs text-stone-400">Ex.: {categoria.exemplos.join(', ')}</p>
                  </div>
                </div>
                {ativo && (
