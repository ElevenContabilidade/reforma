 ...
 * às vezes listam apenas o capítulo/posição, ex.: "02.01").
 */
export function encontrarItemAnexo(codigo: string, tipo: 'NCM' | 'NBS'): AnexoItemRow | undefined {
...
export function montarFicha(codigo: string, descricao: string, tipo: 'NCM' | 'NBS'): FichaTributaria {
  const itemAnexo = encontrarItemAnexo(codigo, tipo) ?? null;
  if (itemAnexo) {
...
/** Expande os tokens de busca com sinônimos conhecidos (busca por OR entre o termo original e o sinônimo). */
...
export function buscarNbs(termo: string): NbsRow[] {
  ...
  for (const item of NBS) {
    if (/^[\d.]+$/.test(termoTrim) && item.codigo.startsWith(termoTrim)) {
      porCodigo.push(item);
    } else if (descricaoCasaComBusca(gruposBusca, tokenizar(item.descricao))) {
      porDescricao.push(item);
    }
